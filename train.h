...
public:
	...
	*/
	...
	template <typename SUBNET> using level1 = res<256, res<256, res<256, res<256, res<256, res_down<256, SUBNET>>>>>>;
	template <typename SUBNET> using level2 = res<128, res<128, res<128, res_down<128, SUBNET>>>>;
	template <typename SUBNET> using level3 = res<64, res<64, res<64, SUBNET>>>;
	template <typename SUBNET> using level4 = res<32, res<32, res<32, SUBNET>>>;
	...
	void regular_load(int batchSize, dlib::rand& rnd, std::vector<frames>& sortedFrames, std::vector<matrix<rgb_pixel>>& images, std::vector<unsigned long>& labels);
